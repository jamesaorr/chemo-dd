...
First we considered an **inverted theta-logistic model**, but this had fitting issues due to it's form. Specifically estimates for r (maximum per capita growth rate) could not be equal to or above observations of dilution rate due to the term $(1 - \frac{g}{r})^\frac{1}{\theta}$. Various transformations and incorporation of measurement error improved things but posteriors for theta were still skewed/bounded. 

We then used a simple **power model** that had a single parameter that determined the shape of density dependence, just like the theta logistic model. This model fit the data very well so we use that to quantify the superlinear shape of density dependence we observed in our data.
...
To begin with we'll use `chemo_dd_eqs_av`, which we believe is the most sensible way of selecting equilibrium points. Once our modelling framework is decided, we'll run the analyses with all the different "final" datasets. This dataset contains 30 observations and has no pseudoreplication. `experiment_replicate` is a grouping factor, but we can't use this as a random effect as only one of the experiments (dd7) tested dilution rates across the full range of dilution rates. 
...
ggplot(chemo_dd_eqs_av, aes(y = od_blanked, 
  ...
  theme_minimal()
```
We'll use Gamma error distribution with the identity link to account for the fact that we can't have negative OD values. 

**Priors** 

$\theta$ describes the curvature of the model, $r$ describes the intercept with the x axis (maximum growth rate), $K$ describes the intercept with the y axis (carrying capacity). Based on this prior knowledge, we can provide some semi-informative priors from visually inspecting the data. 

- We'll say that $r$ will fall somewhere between 0.9 and 1.2 

- We'll say that $K$ will fall somewhere between 0.1 and 0.25

- We'll give loose priors for $\theta$, as this is the main parameter we're interest in and don't want to potentially bias the results. So we'll say it falls somewhere between -0.5 (highly sublinear) and 5 (highly superlinear).
...
where $r$ is now $\frac{\alpha}{\theta}$. The inverse of this function is:  
...
*Just a note, I had also looked at simple polynomial models and a cubic model outperformed a quadratic one and linear one. Cubic model fit the data a lot better than the flipped theta-logistic (not a very flexible model). Very simplistic way of modelling curvature - with linear model with polynomial terms. Problem was that the cubic (which allowed saturation) didn't have a single parameter that described curvature* 
...
predicted_data <- as.data.frame(conditional_effects_data$dilution_rate)
posteriors <- posterior_samples(f_power_model)
a <- mean(posteriors$`b_a_Intercept`)
b <- mean(posteriors$`b_b_Intercept`)          
c <- mean(posteriors$`b_c_Intercept`)

# Power model for density dependence with estimated parameters
od_seq <- seq(0, 0.25, by = 0.001)
...
```
Flipped power model vs power model both give the same curvature (b less than 1, so superlinear). The saturating part of the curve is flipped between the two models, the flipped power model aligns close with the flipped theta logistic - so when you invert these, you get saturating point at maximum growth and low densities - this biologically makes the most sense to me, there must be such low densities that individuals aren't impacted by each other. However, the difference is really minor and probably not all that important given that we are just after the general shape (linear, superlinear, or sublinear). 
...
Fit the theta logistic to my data directly - theta will mean the same. R and K will be inverted. 
...
model2 <- brm(
  ...
  # model formula (parameters to fit are identified and nl flag is set)
  bf(
    od_blanked ~ r * (1 - (dilution_rate / K)^ theta),
    K ~ 1, 
    r ~ 1, 
    theta ~ 1, 
    nl = TRUE
  ...
  # priors  (K and r have been swapped for this model)
  prior = c(
    prior(uniform(0.9, 1.2), nlpar = "K", lb = 0.9, ub = 1.2),  
    prior(uniform(0.10, 0.25), nlpar = "r", lb = 0.10, ub = 0.25), 
    prior(uniform(-0.5, 5), nlpar = "theta", lb = -0.5, ub = 5)  
  ),
...
predicted_data <- as.data.frame(conditional_effects_data$dilution_rate)
posteriors <- posterior_samples(model2)
r <- mean(posteriors$`b_r_Intercept`)
K <- mean(posteriors$`b_K_Intercept`)          
theta <- mean(posteriors$`b_theta_Intercept`)

# Power model for density dependence with estimated parameters
od_seq <- seq(0, 0.25, by = 0.001)
dilution_curve <- K * (1 - (od_seq / r)) ^ (1 / theta)
curve_data <- data.frame(od_blanked = od_seq, 
                         dilution_rate = dilution_curve)
curve_data <- curve_data %>% filter(dilution_rate > 0)
...
As all we're after is an estimate for theta, maybe it is fine to fit it this way? We've fitted the actual theta-logistic model to the inverted data. This means that r and K have opposite meanings, but theta is still the same. For the density dependence persepctive, you just take the estimated parameters and plug them into the flipped theta logistic model. 

Here theta is even more clearly above one than the theta from the flipped theta model (mean of 2.46, error of 0.49, l-95% CI of 1.66, and u-95% CI or 3.56).
