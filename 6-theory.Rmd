...
We set $m = d$ as we assume dilution is the only source of consumer mortality for us. 

We have empirical estimates for $\mu_{max}$, $k$, and $Q$. We know the dilution rates we used and we are simulating $N$ and $R$. That just leaves $S$ - the supply resource concentration. We used 0.05% glucose - so we're pretty sure $S$ is just 0.05.

***The Q that I have estimates for is actually the yield (1/q)***


### Abrams approach simulation

First specify the consumer resource model 
...
t <- seq(0, 1000, 50)
initial_conditions <- c(N = 0.001, R = 0.001)

# Parameters
params <- list(
  mu_max = exp(-0.383)*20,  # Maximum growth rate
  k = exp(-7.86),           # Half-saturation constant
  S = 0.05,                 # Resource supply concentration
  Q = 1/exp(1.33),          # Resource quota
  d = 0.01
)
...
dil_values <- seq(0.01, exp(-0.383), by = 0.0005)
