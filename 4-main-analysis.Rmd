...
This notebook uses empirically estimated Monod functions from previous work by Alicia, Andrew, Jan, and Daniel to make a prediction for the shape of density dependence of E. coli grown under different resource regimes. It is unlikely that the theoretical prediction will align perfectly with the empirical results from the chemostat experiments as these Monod functions are estimated from batch cultures. However, we would expect the general shape (concave vs convex vs nonmonotonic) to be the same.
...
monod_fit <- readRDS(paste0('daniel/daniel_data/fits/MG1655_MCMC_holling_random.Rdata'))
...
# Plot Monod function
p1 <- ggplot() +
  geom_line(data = posterior_curves, aes(x = R, y = mu, group = draw), color = "blue", alpha = 0.05) +
  geom_line(data = monod_data, aes(x = R, y = mu), size = 1) +  
...
## Observed density dependence

In silico experiments across a range of dilution rates 
...
Plot results and compare to analytical prediction
...
Now that we know analytical prediction matches simulations, make analytical prediction that incorporates uncertainty from Monod parameter estimates. 
...
# Expand grid of draws × dilution values
pred_curves <- posterior_subset %>%
...
# Plot results
ggplot() +
  geom_line(data = pred_curves, aes(x = dilution_rate, y = N_star, group = draw),
...
Save theoretical prediction and compare these to empirical results.  

```{r}
write.csv(pred_curves, "data/theory/prediction_posteriors.csv", row.names = F)
write.csv(prediction, "data/theory/prediction.csv", row.names = F)

#rm(pred_curves, prediction)
```
...
# remove observations where m goes above mu_max
...
Really cool - you can see where the Monod parameters make a difference (up at the inflection point, before chemostat dynamics control shape of density dependence). 

I had to be very careful to ensure that for all predictions m was below mu_max, otherwise the function makes weird predictions so the posterior lines jump around the place. 
...
# Loop over dilution and mortality
...
rm(df_chemo, df_chemo_posts, p, plot_list, dil, dil_vals, 
   k, m_vals, mort, q, S, u, plot_subset)
  
```

Only need to plot one dilution rate, but how to get three mortalities onto the same plot? Scale y axis to between mu max and mortality? Scale x axis to be between 0 and carrying capacity? Should call these y axes population growth rate, so as not to confuse with intrinsic mortality of the system. 
...
**Need to check this with Andrew**
...
It is very hard to find the right parameter values for the logistic growth that lead to stable equilibria (no extinctions and no cycles) over a range of mortality values. The quota and r determine the equilibrium abundance but don't change the shape of density dependence. Keeping the Monod function fixed, it is the carrying capacity of the logistic model that controls the shape of density dependence. The just controls which part of the functional response is being observed really. Pick one of these and vary mortality to show the same effect. Increasing K too much leads to cycles at low mortalities. Decreasing K too much leads to extinction at high moralities. Very fine range of parameters. 


Simulate dynamics to find stable parameters 
...
As a first test, confirm that growth under a single pulse of resources leads to superlinear density dependence for type II consumers. Add resources at the start, and watch the consumers grow and the resources go down. 

The abrams method doens't work so well here as the system isn't at an equilibrium. Instead we can get per capita growth rates from the model directly and plot these against consumer density. Focus on the growth stage, how is per capita growth in that stage impacted by density? 
...
What happens when you have multiple pulses? 
...
Important to stress that the density dependence calculations for this pulsed resources is only valid if all of the resources are depeleted between pulses. If not all of the resources are depleted, then the resources are more like chemostat and the density dependence of the resource will be inherited by the consumer. Simulations with shorter pulse intervals won't show this because per capita growth rate is being calculated from the consumer equation and is ignoring the resource. Important to be clear about this assumption. 



### Plot by mortality 

If we want to have three panels from three different moralities and we want all three resource dynamics one each one, we'll need to rescale the axes so that comparisons are fair. Y axis could be scaled between maximum observed per capita growth and mortality. X axis could be scaled between 0 and carrying capacity. The other option is to have a 3 x 3 panel with all combinations of mortality and resource dynamics. 

As the three resources dynamics lead to three different observed maximum growth rates, we can't give all three the same exact mortality values to represent high, medium, and low mortalities. A straightforward solution would be to express mortality as a percentage of the observed maximum growth rate. Choosing something like 25%, 50%, and 75%  mortality (relative to max growth) shows the strong impact of mortality on the observed density dependence. 
...
S <- 0.05
df_chemo <- expand.grid(m = m_vals, dil = dil_vals) %>%
  mutate(avg_od = N_star_chemo(dil, m, u, S, k, q)) %>%
  mutate(pcgr = m) %>%
  filter(avg_od > 0)
# generate uncertainty 
m_vals <- seq(0.01, max(posterior_subset$mu_max), length = 1000) 
df_chemo_posts <- posterior_subset %>%
  ...
  crossing(m = m_vals, dil = dil_vals) %>%
  mutate(N = N_star_chemo(dil, m, mu_max, S, Ks, q))
# remove observations where m goes above mu_max and negative N
df_chemo_posts <- df_chemo_posts %>%
  filter(m < mu_max) %>%
  filter(N > 0)
...
K_vals <- 0.0004
df_log <- expand.grid(m = m_vals, K = K_vals) %>%
  mutate(avg_od = N_star_log(r, K, m, u, k, q)) %>%
  mutate(pcgr = m) %>%
  filter(avg_od > 0)
# generate uncertainty 
m_vals <- seq(0, max(posterior_subset$mu_max), length = 1000) 
...
df_pulse <- output_df %>%
  mutate(pcgr = (params$mu_max * R / (params$k + R)) - params$m) %>%
  rename(avg_od = N) %>%    # to be consistent with chemo and log 
  filter(avg_od > 0)
# generate uncertainty 
...
df_chemo_posts <- df_chemo_posts %>%
  mutate(pcgr = m) 
df_log_posts <- df_log_posts %>%
  mutate(pcgr = m) 
df_pulse_posts <- df_pulse_posts %>%
  mutate(m = pcgr) 
df_chemo <- df_chemo %>%
  mutate(N = avg_od) 
df_log <- df_log %>%
...
```
Rescaling the posteriors seems to get rid of their variation. I have to do the rescalign within a draw, so it isn't all that surprising I guess. Definitely leaning more towards the 3 x 3 plot. 
