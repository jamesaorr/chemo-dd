...
chibio data only for dd7 at the moment
...
### Select sampling points for the equilibrium estimates
...
# Select final day that is relevant
# Low dilution, day 4
# Mid dilutions, day 2
# High dilutions, day 4
chemo_dd_eqs_fin <- chemo_dd %>%
  filter(
    (dilution_rate < 0.1 & day == 4) |
    (dilution_rate > 0.7 & day == 4) |
    (dilution_rate < 0.7 & dilution_rate > 0.1 & day == 2)
  )

# Select day 4 for all (assumes that evolution not important)
...
write.csv(chemo_dd_eqs_fin, "data/processed/equilibrium_samples_fin.csv", row.names = F)
