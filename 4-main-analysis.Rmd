...
This notebook uses the processed data from `1-data-prep.Rmd` and the estimated Monod parameters for the *E. coli* strain used in our study (in the fits sub folder) to estimate the shape of density dependence of *E. coli* growing on glucose.
...
## Analytical growth-density inversion on parameterized consumer-resource model
...
$\mu_{max}$ = maximum growth rate of consumer

$k$ = half saturation constant of consumer
...
We can start by simulating the consumer resource model under different dilution rates (where m = d) and plot equilibrium abundances against dilution rates. We can also analytically predict the shape of density dependence that we expect to see when d = m. We set the left hand side of the equations to zero and solve for $N$ and $R$. From the consumer equation:
...
mu_med <- exp(sumparams$median)[1]
k_med <- exp(sumparams$median)[2]
q_med <- 1/exp(sumparams$median)[3] # q in this model is actually yield, so we need to convert it to quota (inverse)
...
Now we can plug in the parameter estimates to the analytical growth-density inversion solution for our consumer-resoruce model (equation above).
...
ggplot() +
  ...
  geom_line(data = posterior_curves, aes(x = R, y = mu, group = draw), 
            alpha = 0.25, size = 0.5, color = "gray50") +
  
  geom_line(data = monod_data, aes(x = R, y = mu), 
            size = 1, alpha = 1, color = "black") +  
  
  labs(x = "Glucose Concentration (mg/mL)", y = "Per Capita Growth (hour ^-1)") +
  
  # R is % glucose in our model, 1% glucose is 10mg/mL
...
ggplot() +
  ...
  geom_line(
    ...
    aes(x = N_star, y = dilution_rate, group = draw),
    color = j_blue, size = 0.25, linetype = "solid", alpha = 0.15) +

  geom_line(
    data = prediction,
    aes(x = N_star, y = dilution_rate),
    color = j_blue, size = 1, linetype = "solid", alpha = 1) +

  labs(y = "Per Capita Growth", x = "Density") +
...
## Empirical growth-density inversion - chemostat experiment

Above we analytically applied growth-density inversion to a parameterized consumer-resource model of our system. Now we can just analyze the data generated in our chemostat experiment where we manipulated dilution rates and quantified equilibrium densities to see if we see a similar shape of density dependence. We will estimate the shape of density-dependence in a phenomenological way using the classic theta-logistic model that is familiar to those who study density dependence. 

### Fitting inverse theta-logistic

As we experimentally controlled growth rate rather than density, we'll fit the inverse of the theta logistic model: 

$$
N=K\left(1-\frac{g}{r}\right)^{\frac{1}{\theta}}
$$

See `3-supplementary-models.Rmd` for details on alternative models and fitting approaches. These other modelling approaches (e.g., inverse theta logistic in `brms`, power model) find superlinearity as well. Here, we'll use a grid based approach that is very transparent and that can overcome the boundary issues we had with the MCMC algorithms in `brms`, where the parameter r can't be lower than the observed g values. 
...
grid <- crossing(K = seq(0.1, 0.5, length.out = 50),
                 r = seq(0.5, 1.5, length.out = 50),
...
Next, we add some priors that are very broad but that include values that are biologically possible in our system. More discussion of priors is available in `3-supplementary-models.Rmd`. 
...
grid <- grid |>
  mutate(
    log_prior_K = dunif(K, min = 0.1, max = 0.5, log = TRUE),
    log_prior_r = dunif(r, min = 0.5, max = 1.5, log = TRUE),
    log_prior_theta = dunif(theta, min = 0.1, max = 10, log = TRUE),
    log_prior_sigma = dunif(sigma, min = 0.01, max = 0.05, log = TRUE),
...
The grid above (50 x 50 x 50 x 20) has 2.5 million combinations of parameters, so it takes quite a while to run (~10 mins on an M1 MacBook Pro), so we have the option to just load the grid from a previous run instead:
...
Evidence of slight bimodality (theta logistic not flexible enough to perfectly fit the density dependence we see in our experiment), which is also obvious in the plots below. However, it is a totally adequate model to statistically verify that the shape of density dependence is superlinear.

We can randomly draw parameter combinations from the posterior distribution (weighted by their posterior probabilities) and then plot the resulting estimated curve for each of them:
...
ggplot(data = exp_data, aes(y = Neq_dat, x = g)) +
  labs(
    y = "Optical Density", x = "Dilution rate") +
  ...
  scale_x_continuous(breaks = seq(0, 0.9, by = 0.3),
                     limits = c(0, 0.94)) +
...
## Analytical predictions across alternative resource dynamics
...
Through the analytical growth-density inversion approach (outlined more clearly above) we get: 
...
The shape of density dependence will really depend on the baseline mortality of the system. If there is no mortality, then per capita growth is just the Monod function. Adding in baseline mortality will change the window of density dependence that is being viewed. The parts of the density dependence curves above the dashed lines show you density dependence under three different baseline mortalities (for the three dilutions).
...
plot_subset <- function(dilution, mort) {
  ...
  # prediction with medians of parameters
  med_pred <- df_chemo %>% 
    filter(dil == dilution) %>%
    filter(N > 0) %>%
    filter(m > mort)
  ...
  # plot together
  p <- ggplot() +
    geom_line(data = med_pred, aes(y = m, x = N), size = 1) +
...
Dilution rate only changes the x axis range so we can just plot one dilution rate.
...
Again, we can use an analytical growth-density inversion approach to predict the equilibrium density of the consumer for this particular model. 
...
Growth-density inversion doesn't work so well here as the system doesn't ever reach a fixed point equilibrium (pulsed resources lead to cycling dynamics for the consumer). Instead we can get per capita growth rates from the model directly and plot these against consumer density. 
...
# Prediction with median estimates
...
```
Note that if we use multiple pulses the same results hold. Similarly, the interval between pulses doesn't matter. Even short intervals where the resources aren't depleted fully between pulses doesn't turn the system into a chemostat style resources. There is no resource loss here other than through the consumers. Therefore there is no density dependence of resources for the consumers to "inherit" (sensu Abrams' principle of inheritance of the curvature). 
...
plot_subset <- function(data, data2, mort_value) {
  ...
  p <- ggplot() +
    ...
    geom_line(data = med_pred, aes(y = pcgr_realised, x = N), colour = j_blue) +
    labs(y = "", x = "") +
    theme_minimal() +
