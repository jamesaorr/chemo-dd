...
This notebook uses the processed data from `1-organation.Rmd` for modelling
...
For this first attempt I'll use OD and dilution rate from `equilibrium_samples` - 31 observations, no pseudoreplication. `experiment_replicate` may be a grouping factor that I can use for multi-level models (but probably not enough points per experiment). We manipulated dilution rate (we can be confident that there is no/low error around these values) and OD is our response variable. This means we are kind of flipping the theta-logistic model on its head. 
...
model <- brm(
  ...
  bf(
    avg_od_blanked ~ K * (1 - (abs(dilution_rate / r))) ^ (1 / theta),
    K ~ 1,  
    r ~ 1,
    theta ~ 1, 
    nl = TRUE  
  ...
  prior = c(
    ...
    prior(uniform(0.10, 0.25), nlpar = "K", lb = 0.1, ub = 0.25), 
  ...
  # inital values to help with start up if needed
...
Lots of divergence and weird posterior distribution of r - upon closer inspection, r is never able to be equal to or lower than the maximum of `dilution_rate`. 

Theta-logistic model is not flexible enough to give a perfect fit to the data (especially with undefined issue below). It fits pretty well, but if I choose values for r and K that I expect, the theta parameter can't be fine-tuned to get a "perfect" fit (see plots below). 

Plot the model against the points (and include a model with user-defined parameters)
...
#### fitting theta-logistic model with flipped axes

I should probably disregard this as regression is assymetric, in the sense that it really matters what is the response and what is the predictor in terms of how variance is expained. It is assuemd that the predicts are known with certainty but that there is uncertainty (residules) around the response variable. Our experiment has OD as the response variable and dilution_rate as the predictor variable - so we have to model them that way around. No point looking at the proper theta-logistic model. 
...
I've tried using exp(log_r), exp(r), and inv_v - none of these seemed to make a difference

Another thought I had was to reparameterize theta so that it wasn't 1/theta, but that it is just inv_theta - priors from 1/-0.5 to 1/5 - see what happens, may also need to be log transformed? It seemed to have a minor effect, look into this more.. Discuss with Andrew. 
...
model3 <- brm(
  ...
  prior = c(
    ...
    prior(uniform(-0.5, 5), nlpar = "theta", lb = -0.5, ub = 5),
    #prior(uniform(0.2, 10), nlpar = "invtheta", lb = 0.2, ub = 10)
...
```{r}
loo(linear_model, quad_model, quad_model_random)
```
Seems like quadratic without random effects is best. 
