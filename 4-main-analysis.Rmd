...
```
The grid above (50 x 150 x 50 x 20) has 7.5 million combinations of parameters, so it takes quite a while to run (~30 mins on an M1 MacBook Pro), so we can just load the grid from a previous run instead:
...
Evidence of bimodality (theta logistic not flexible enough to fit the density dependence we see in our experiemnt), which is also obvious in the plots below.
