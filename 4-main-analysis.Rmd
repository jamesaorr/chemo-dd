...
## Prediction from consumer-resource model
...
grid <- crossing(K = seq(0.15, 0.25, length.out = 50),
                 r = seq(0.6, 1.1, length.out = 50),
                 theta = seq(-1, 10, length.out = 50),
...
grid <- grid |>
  mutate(
    ...
    log_prior_r = dunif(r, min = 0.6, max = 1.1, log = TRUE),
    log_prior_theta = dunif(theta, min = -1, max = 10, log = TRUE),
...
ggplot(data = exp_data, aes(y = Neq_dat, x = g)) +
  ...
  geom_line(data = curve_data, 
            aes(x = g, y = N), 
            col = j_green, size = 1, linetype = "dashed") +
