...
Simple quadratic model, with default priors 
...
loo_quad <- loo(quad_model)
loo_compare(loo_linear, loo_quad)
```

elpd_diff shows the difference in ELPD between two models. The model with the higher ELPD (lower elpd_diff) is considered to have better predictive performance. Quadratic model the better model (obviously). 