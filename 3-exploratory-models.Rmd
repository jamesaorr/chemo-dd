...
This notebook uses the processed data from `1-organation.Rmd` for preliminary modelling. We explore various approaches to quantify the curvature of the density-dependence.
...
p1 <- ggplot(chemo_dd_eqs_av, aes(y = od_blanked, x = dilution_rate)) +
  labs(
    y = "Density (OD)", x = "Dilution rate (ml/hour/ml)") +
...
p2 <- ggplot(data.frame(theta = theta_samples), aes(x = theta)) +
  geom_histogram(binwidth = 0.025, fill = "#185266", color = NA, alpha = 0.7) +
  labs(
    x = expression(theta),
    y = "Frequency"
  ) +
  scale_x_log10(
    ...
    labels = scales::label_number()     # Format labels as numbers
  ) +
  theme_light(base_size = 25) 
