...
The grid above (50 x 150 x 50 x 20) has 7.5 million combinations of parameters, so it takes quite a while to run (~30 mins on an M1 MacBook Pro), so we can just load the grid from a previous run as well: 
...
Median prediction (based on median parameter values) and 95% credible intervals (based on predictions from posterior predictive distribution). Option to use HDI or quantile based credible intervals.  
