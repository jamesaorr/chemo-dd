...
outflow <- outflow %>%
  select(c(bioreactor, day_measured, volume_out, experiment_rep, time_elapsed)) %>%
  mutate(outflow_per_hour = volume_out/time_elapsed) %>%
  group_by(bioreactor, experiment_rep) %>%
...
CC <- CC %>%
  left_join(outflow, by = c("ID" = "bioreactor", "experiment_replicate" = "experiment_rep"))
```


Join in the OD data (does this need to be "blanked" - should we minus everything by control)
